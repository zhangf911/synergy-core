...
ArchNetworkWinsock::bindSocket (ArchSocket s, ArchNetAddress addr) {
    ...
    if (bind_winsock (s->m_socket,
                      TYPED_ADDR (struct sockaddr, addr),
...
ArchNetworkWinsock::acceptSocket (ArchSocket s, ArchNetAddress* const addr) {
    ...
    ArchSocketImpl* socket = new ArchSocketImpl;
    ArchNetAddress tmp =
        ArchNetAddressImpl::alloc (sizeof (struct sockaddr_in6));
    ...
    switch (family) {
        ...
        case kINET6: {
            ...
            struct sockaddr_in6* ipAddr =
                TYPED_ADDR (struct sockaddr_in6, addr);
...
ArchNetworkWinsock::addrToName (ArchNetAddress addr) {
    ...
    char host[1024];
    char service[20];
    int ret = getnameinfo (TYPED_ADDR (struct sockaddr, addr),
                           addr->m_len,
                           host,
                           sizeof (host),
                           service,
